...
class has_ℂ_inner_product (α : Type*) := (inner_product : α → α → ℂ)

export has_ℂ_inner_product (inner_product)

class ℂ_inner_product_space (α : Type*) [add_comm_group α] [vector_space ℂ α] extends has_ℂ_inner_product α :=
...
by apply ℂ_inner_product_space.conj_symm

lemma linearity (x y z : α) (a : ℂ) : (a • x + y) † z = a * (x † z) + y † z :=
by apply ℂ_inner_product_space.linearity
...
by apply ℂ_inner_product_space.pos_def
...
@[simp] lemma mul_re_in_fst (x y : α) (r : ℝ) : (((r : ℂ)•x)†y).re = r * (x†y).re :=
begin
    simp,
end

@[simp] lemma mul_re_in_snd (x y : α) (r : ℝ) : (x†((r : ℂ)•y)).re = r * (x†y).re :=
begin
    have w := conj_symm x ((r : ℂ)•y),
    rw [w],
    simp,
end
...
begin
    dsimp [ip_self],
    rw [←@neg_one_smul ℂ α _ _ _ x],
    simp only [mul_in_fst_coord, mul_in_snd_coord],
    have w : conj (-1) = -1 := by simp,
    rw [w],
    simp,
end
